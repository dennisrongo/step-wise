...
- **One updater key, all platforms.** The app verifies every platform's update against the single `pubkey` in `src-tauri/tauri.conf.json`. So the Windows machine must sign with a **copy of the same** `~/.tauri/stepwise-updater.key` the Mac uses (`TAURI_SIGNING_PRIVATE_KEY`). The key is a secret — copy it over, never commit it.
