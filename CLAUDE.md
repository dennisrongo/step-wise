...
- **Health data (`health/`):** behind a real Google source (`google.rs`) and a Demo source (`demo.rs`); demo is selected by `STEPWISE_DEMO=1`. **Steps** are the core metric (daily roll-up + intraday hourly) and the only *required* fetch — if steps fail the whole week fails and the UI shows the error. **Distance** and **active minutes** ride the same `activity_and_fitness` scope; **resting HR** (`health_metrics_and_measurements` scope) and **sleep** (`sleep` scope, via the `list` method — it has no `dailyRollUp`) need their own scopes, so they only populate after the user reconnects to grant them. Every metric except steps is **best-effort** (`optional()`): a per-metric failure (missing scope, transient error, unexpected shape) degrades to `None` (rendered `—`) and is logged — it never blanks the panel. Never fabricate a value. HR/sleep wire shapes are the least documented; `fetch_daily_resting_hr`/`fetch_sleep` log the raw response at `debug` to confirm fields against live data.
