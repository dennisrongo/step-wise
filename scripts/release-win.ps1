...
function Invoke-Preflight {
  ...
  if ([string]::IsNullOrWhiteSpace($key)) {
    Add-Check 'Updater key' 'FAIL' 'TAURI_SIGNING_PRIVATE_KEY not set' 'Copy ~/.tauri/stepwise-updater.key from the Mac, then set TAURI_SIGNING_PRIVATE_KEY in .env (path or base64 contents).'
  ...
  if (-not (Has-Command 'git')) {
    ...
    if ($LASTEXITCODE -ne 0) {
      Add-Check 'Git repo' 'FAIL' '' 'Run this from inside the stepwise clone.'
    } else {
